...
// Retrieve entry file mappings by function recursion（通过递归函数检索入口文件映射）
const walk = (dir) => {
	dir = dir || '.';
	...
	fs.readdirSync(directory).forEach((file) => {
		...
		if (stat.isFile() && extname === '.vue' || extname === '.we') {
			...
			if (extname === '.vue') {
				...
				//我们现在这里插入html-webpack-plugin
				if (name.endsWith('index')){
					plugins[plugins.length] = new HtmlWebpackPlugin({
						filename: name + '.html',
						template:'./template.html',
						inject: true,              // js插入位置
						chunks: [name], // 每个html引用的js模块
					})
				}
			}
...
// Generate an entry file before writing a webpack configuration（编写webpack配置前关联一个入口文件）
walk();
